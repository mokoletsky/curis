...
class DataLoader:
    def __init__(self, args, input_files={'txt': None, 'label': None}, input_text=None, input_data=None, vocab=None, evaluation=False, dict={}):
        ...
        with open("/sailhome/gsychi/test/stanza/thai-syllable.json", "r") as fp:
            self.num_syllables = json.load(fp)
        #print(self.num_syllables)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 
    ...
    def next(self, eval_offsets=None, unit_dropout=0.0, old_batch=None, feat_unit_dropout=0.0):
        ...
        for i, (offset, pair) in enumerate(offsets_pairs):
            ...
            temp_txt = ''.join(r_)
            temp_txt = syllable_tokenize(temp_txt)
            for j in range(len(temp_txt)):
                try:
                    syllable_units[i][j] = self.num_syllables[temp_txt[j]] + 2
                except:
                    syllable_units[i][j] = 1
